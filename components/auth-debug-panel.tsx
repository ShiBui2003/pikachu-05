...
export default function AuthDebugPanel() {
  ...
  const clearAllAuth = async () => {
    if (typeof window !== 'undefined') {
      ...
      // Clear cookies
      const cookies = ['sb-access-token', 'sb-refresh-token', 'sb-provider-token'];
        ...
        <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {debugInfo.user && (
                <div className="text-sm space-y-1">
                  ...
                  <p><strong>Role:</strong> {debugInfo.user.user_metadata?.role || 'Not set'}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={debugInfo.session ? "default" : "destructive"}>
                  {debugInfo.session ? "Active" : "No Session"}
                </Badge>
              </div>
              {debugInfo.session && (
                <div className="text-sm space-y-1">
                  <p><strong>Expires:</strong> {new Date(debugInfo.session.expires_at * 1000).toLocaleString()}</p>
                  <p><strong>Token Type:</strong> {debugInfo.session.token_type}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {Object.entries(debugInfo.cookies).map(([name, value]) => (
                <div key={name} className="text-xs">
                  <strong>{name}:</strong> {maskSensitiveValue(value, showSensitive)}
                </div>
              ))}
          ...
          <div>
            <h3 className="font-medium mb-2">
              LocalStorage ({Object.keys(debugInfo.localStorage).length})
            </h3>
            <div className="space-y-1 max-h-32 overflow-y-auto">
