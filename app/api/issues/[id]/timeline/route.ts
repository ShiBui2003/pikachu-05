...
export async function GET(
  ...
  try {
    ...
    timelineEvents.push({
      ...
      description: 'Your issue has been submitted and is awaiting review by our team.',
    ...
    try {
      ...
      if (updates) {
        updates.forEach(update => {
          ...
          let description = `Issue status changed to ${update.status?.replace('_', ' ')}`;

          switch (update.status) {
            case 'assigned':
              title = '✅ Issue Accepted';
              description = 'Great news! Your issue has been accepted and assigned to a department for resolution.';
              break;
            case 'in_progress':
              title = '🔧 Work Started';
              description = 'Our team has started working on your issue. We\'ll keep you updated on the progress.';
              break;
            case 'resolved':
              title = '🎉 Issue Resolved';
              description = 'Your issue has been resolved! Please check if the problem has been fixed.';
              break;
            case 'closed':
              title = 'Issue Closed';
        ...
        workflowStates.forEach(state => {
          if (state.notes || state.estimated_completion) {
            timelineEvents.push({
              ...
              description: 'Admin has provided additional information about your issue.',
              comment: state.notes,
    ...
    try {
      ...
      if (adminNotifications) {
        adminNotifications.forEach(notification => {
          ...
          let description = 'An admin has taken action on your issue.';

          switch (notification.action) {
            case 'department_assigned':
              title = '🏢 Department Assigned';
              description = `Your issue has been assigned to ${notification.details?.department_name || 'a department'}.`;
              break;
            case 'priority_changed':
              title = '⚡ Priority Updated';
              description = `Issue priority has been updated to ${notification.details?.priority || 'updated'}.`;
              break;
    ...
    if (latestEvent && latestEvent.status !== issue.status) {
      ...
      let description = `Issue is currently ${issue.status?.replace('_', ' ')}`;

      switch (issue.status) {
        case 'assigned':
          title = '📋 Currently Assigned';
          description = 'Your issue is currently assigned and awaiting action.';
          break;
        case 'in_progress':
          title = '⚙️ Work in Progress';
          description = 'Our team is actively working on resolving your issue.';
          break;
        case 'resolved':
          title = '✅ Resolved';
