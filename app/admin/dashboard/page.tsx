...
const AdminDashboard = () => {
  ...
  const navItems = [
    { id: "dashboard", label: "Dashboard", icon: LayoutDashboard, badge: null, route: "/admin/dashboard" },
    { id: "reports", label: "Reports", icon: FileText, badge: "23", route: "/admin/reports" },
    { id: "bidding", label: "Bidding", icon: Gavel, badge: "3", route: "/admin/bidding" },
    { id: "zones", label: "Zones", icon: Map, badge: "2", route: "/admin/zones" },
    { id: "wards", label: "Wards", icon: Map, badge: "5", route: "/admin/wards" },
    { id: "departments", label: "Departments", icon: Building, badge: null, route: "/admin/departments" },
    { id: "export", label: "Export Report", icon: Download, badge: null, route: "/admin/export" },
    { id: "settings", label: "Settings", icon: Settings, badge: null, route: "/admin/settings" },
  ]
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [statsResult, categoriesResult, trendsResult, departmentsResult, recentResult, notificationsResult] =
        await Promise.all([
          fetchOverviewStats(),
          fetchCategoryData(),
          fetchMonthlyTrends(),
  ...
  const fetchOverviewStats = async (): Promise<DashboardStats> => {
    ...
    const pendingIssues = typedIssues.filter((i) => i.status === "submitted").length || 0
    const inProgressIssues = typedIssues.filter((i) => ["in-review", "in-progress"].includes(i.status)).length || 0
    const resolvedIssues = typedIssues.filter((i) => i.status === "resolved").length || 0

    const newThisWeek = typedIssues.filter((i) => new Date(i.created_at) >= oneWeekAgo).length || 0

    const resolvedThisWeek =
      typedIssues.filter((i) => i.status === "resolved" && i.updated_at && new Date(i.updated_at) >= oneWeekAgo)
        .length || 0

    const resolvedWithTimes = typedIssues.filter((i) => i.status === "resolved" && i.updated_at) || []
    ...
    const citizenSatisfaction = resolvedIssues > 0 ? Math.min(95, 70 + (resolvedIssues / totalIssues) * 25) : 0
  ...
  const fetchCategoryData = async () => {
    const { data: issues, error } = await supabase.from("issues").select("category")
    ...
    const categoryCounts = typedIssues.reduce((acc: { [key: string]: number }, issue) => {
  ...
  const fetchMonthlyTrends = async () => {
    const { data: issues, error } = await supabase.from("issues").select("created_at, updated_at, status")
    ...
    return months.map((month) => {
      ...
      const resolved =
        typedIssues.filter((issue) => {
          if (issue.status !== "resolved" || !issue.updated_at) return false
  ...
  const fetchDepartmentPerformance = async () => {
    const { data: issues, error } = await supabase.from("issues").select("category, status")
    ...
    const deptStats: { [key: string]: { assigned: number; completed: number } } = {}
    ...
    typedIssues.forEach((issue) => {
      ...
      if (["in-progress", "resolved", "assigned"].includes(issue.status)) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-slate-950 dark:to-slate-900">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white dark:bg-slate-950 border-b">
        <div className="flex items-center justify-between px-4 py-3">
          <h1 className="text-xl font-bold text-accent">JanPath Admin</h1>
        ...
        >
          <div className="p-4 md:p-6 space-y-6">
            ...
            <div className="hidden md:block">
              <h2 className="text-2xl font-bold text-accent mb-2">JanPath</h2>
            ...
            <div className="pt-6 border-t">
              <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center text-white font-bold">
                  {currentUserProfile?.full_name?.charAt(0)?.toUpperCase() || "A"}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium truncate">{currentUserProfile?.full_name || "Admin User"}</p>
                  <p className="text-xs text-muted-foreground truncate">{userDepartmentName || "Super Admin"}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto">
          ...
          <div className="sticky top-0 bg-white dark:bg-slate-950 border-b p-4 md:p-6 flex items-center justify-between z-30 md:z-10">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mt-1">Welcome back! Here's your real-time analytics</p>
            </div>
            <div className="flex items-center gap-4">
              <div className="relative hidden md:block">
                <Search className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground" />
                <Input placeholder="Search..." className="pl-10 w-64 bg-slate-50 dark:bg-slate-900 border-0" />
              </div>
              ...
              {[
                ...
                return (
                  <Card key={idx} className="hover:shadow-lg transition-shadow">
                    <CardContent className={`p-4 md:p-6 rounded-lg ${kpi.color}`}>
                      <div className="flex items-start justify-between">
                        ...
                        <Icon className={`w-8 h-8 ${kpi.textColor} opacity-50`} />
                      </div>
                    </CardContent>
                  </Card>
                )
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {dept.completed} of {dept.assigned} resolved
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-lg text-accent">{dept.efficiency}%</p>
                  ...
                  {recentIssues.map((issue) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{issue.location_address || "No location"}</p>
                      </div>
                      <div className="flex items-center gap-2 ml-4">
                        <Badge className={getStatusColor(issue.status)}>{issue.status}</Badge>
                        {issue.priority && <Badge className={getPriorityColor(issue.priority)}>{issue.priority}</Badge>}
                      </div>
                    </div>
                  ))}
                </div>
