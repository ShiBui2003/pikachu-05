...
export default function CitizenCrowdfundingPage() {
  ...
  const fetchCampaigns = async () => {
    ...
    if (error) console.error(error); else setCampaigns(data || []);
    setLoading(false);
  };
  ...
  const handleDonate = async (amount: number, campaign: Campaign) => {
    ...
    const rzp = new (window as any).Razorpay({
      ...
      handler: async function (response: any) {
        ...
        await supabase.from("campaigns").update({ raised_amount: campaign.raised_amount + amount }).eq("id", campaign.id);
        fetchCampaigns();
      },
      prefill: { name: "", email: "" },
      theme: { color: "#3399cc" }
    ...
    <main className="p-6">
      ...
      {loading ? <p>Loading campaigns...</p> :
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {campaigns.map(c => (
            <div key={c.id} className="border rounded-xl shadow p-4">
              <h2 className="text-xl font-semibold">{c.title}</h2>
              <p className="mt-2 text-sm text-gray-700">{c.description}</p>
              <p className="mt-3 font-medium">Raised: ₹{c.raised_amount} / ₹{c.target_amount}</p>
              <p className="text-xs text-gray-500 mt-1">Posted by: {c.role}</p>
              <button onClick={() => handleDonate(500, c)} className="mt-4 px-4 py-2 bg-green-600 text-white rounded-lg">Donate ₹500</button>
            </div>
          ))}
        </div>
      }
