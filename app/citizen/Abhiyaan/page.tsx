  ...
  return (
    <main className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-10">
        <input type="text" placeholder="Abhiyaan Name" value={name} onChange={(e) => setName(e.target.value)} className="w-full p-2 border rounded" />
        ...
        <input type="text" placeholder="Your Name / Organization" value={creatorName} onChange={(e) => setCreatorName(e.target.value)} className="w-full p-2 border rounded" />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {abhiyaans.map((a) => (
            <div key={a.id} className="border rounded-xl shadow p-4">
              ...
              <p className="mt-1 text-sm">Location: {a.location}</p>
              <p className="mt-1 text-sm">Time: {new Date(a.time).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
